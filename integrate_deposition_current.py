...
def run_script(IDENTIFIER, STRING):
    ...
    if PLOTTING:
        plt.figure(1)
        for i in np.arange(deposition.keys().__len__()):
...
def get_filtered_data(IDENTIFIER, SENSITIVITY_FILTER, PLOTTING, DEBUGGING):
    ...
    #FILTER = np.where(rtime >= 600)
    #rtime, rcurrent = rtime[FILTER], rcurrent[FILTER]
    if PLOTTING:
        plt.figure(1)
...
def get_raw_data(IDENTIFIER):
    db = Cinfdata('omicron', use_caching=False)
...
def get_regions(time, data, first_region, LIMIT, SENSITIVITY_LIMIT, DEBUGGING):
    ...
    print('\nRegions detected:\n\t{} deposition currents and\n\t{} leak currents\n'.format(deposition.keys().__len__(), leak.keys().__len__()))
...
if __name__ == '__main__':
    try:
        run_script(14409, 'PARTICLE_DIAMETER=5;APERTURE_DIAMETER=9.;FIRST_LIMIT=14.0;SENSITIVITY_LIMIT=0.7;SENSITIVITY_FILTER=10.;PLOT=True;DEBUG=rue')
    except:
